...
for ($i = 1; $i <= $totalRating; $i++) {
 ...
 {   
  $um_id= um_user( 'ID' );
	...
	$user_meta=get_userdata($um_id);
	


$user_roles=$user_meta->roles; 

if($user_roles[0]=="dc_vendor"){
 ...
 <div style="margin-top:30px;"><?php 
						   ...
						   //echo $user->data->ID; 
  $all_meta_for_user = get_user_meta($um_id);
 $id_d=$all_meta_for_user['_vendor_term_id'][0];
     $rating_info = wcmp_get_vendor_review_info($id_d);
...
<!-- EDITED bY ajay soni -->
						<h3 
						   title="<?php echo um_user( 'display_name' ); ?>"><?php
...
<!-- end EDITED bY ajay soni -->
	...
	<!--<div>
	<?PHP 
	$vendor2 = get_wcmp_vendor(get_current_vendor_id());
	$reviews_lists = $vendor2->get_reviews_and_rating(0);
	{ 
	    
$c_id=$reviews_lists[0]->comment_ID;
$c_r=get_comment_meta($c_id);
$c_r_value=  $c_r['vendor_rating'][0];
	?>
	<div>
	    
					<h3>Past Reviews</h3>	
					<p class='meta'>
					<strong itemprop="author"><?PHP ECHO $reviews_lists[0]->comment_author; ?></strong> <em class='verified'>(verified buyer)</em> – <time itemprop='datePublished' datetime='<?php echo $reviews_lists[0]->comment_date; ?>'>
					    <?php 
					    $timestamp = strtotime($reviews_lists[0]->comment_date);
				     echo human_time_diff($timestamp, current_time('timestamp')).' ago'; ?>
					</time>
				</p>
	<?php 
	$totalRating = 5;
$starRating = $c_r_value;
for ($i = 1; $i <= $totalRating; $i++) {
     if($starRating < $i ) {
        if(is_float($starRating) && (round($starRating) == $i)){
            echo "<i class='fa fa-star-half-o' aria-hidden='true'></i>";
	...
	?>
					<div itemprop='description' class='description'><?php echo $reviews_lists[0]->comment_content; ?> <div style='height:10px; width:100%'>&nbsp;</div></div>
		</div>
	<?php }
	?>
  ...
  <tr>
    ...
    <th>Qty</th> 
    <th>Price</th>
     <th>Date Availabel</th>
     </tr>
	    <?php  
	    $vendor = get_wcmp_vendor(get_current_vendor_id());
	    if ($vendor->get_products()){
                    foreach ($vendor->get_products() as $product) {   $product = wc_get_product($product->ID); 
                       ?>
     <tr>
    <td>Jill</td>
    <td><?php      
...
if( $term = get_term_by( 'id', $id, 'product_cat' ) ){
    ...
    <td><?php  echo $product->price;?></td>
     ...
     $mydate = $product->date_on_sale_from; 
    $mydate =  new DateTime($mydate);
     echo $mydate->format('Y-m-d');
      echo ' To ';
   $mydate2 = $product->date_on_sale_to; 
    $mydate2 =  new DateTime($mydate2);
...
if( $term = get_term_by( 'id', $id, 'product_cat' ) ){
    ...
    ?></td> 
    <td><?php  echo $product->id;  echo $product->name; ?></td>
       <td><?php $product->description; ?></td>
    <td><?php echo '1';  ?></td> 
    <td><?php  echo $product->price;?></td>
     <td><?php  
   ...
   echo ' To ';
    ...
    $mydate2 =  new DateTime($mydate2);
     ...
     <td>
		...
		<a href="/kitchens2tables/shop/?add-to-cart=<?php echo $product->id;?>" data-quantity="1" class="viewcart-style-1 button product_type_simple add_to_cart_button ajax_add_to_cart" 
		data-product_id="<?php echo $product->id;?>" data-product_sku="" aria-label="Add “Black Forest Truffe” to your cart" rel="nofollow">Add to cart</a>
		<div class="quickview" data-id="2996" title="Quick View">Quick View</div>	
		</div>
     </td>
  </tr>
   <?php  }
