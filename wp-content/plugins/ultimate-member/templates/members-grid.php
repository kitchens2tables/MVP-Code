<div class="um-members">
	...
	$iProfileurl = SITE_URL().'/chef-profile/'.$usernickname; 		
	...
	<div class="um-member um-role-<?php echo um_user( 'role' ); ?> <?php echo um_user('account_status'); ?> <?php if ($cover_photos) { echo 'with-cover'; } ?>">
		...
		<div class="um-member-cover" data-ratio="<?php echo UM()->options()->get('profile_cover_ratio'); ?>">
			<div class="um-member-cover-e"><a href="<?php echo $iProfileurl; ?>" title="<?php echo esc_attr(um_user('display_name')); ?>"><?php echo um_user('cover_photo', $cover_size); ?></a></div>
		</div>
		...
		?>
		<div class="um-member-photo radius-<?php echo $corner; ?>"><a href="<?php echo $iProfileurl; ?>" title="<?php echo esc_attr(um_user('display_name')); ?>"><?php echo get_avatar( um_user('ID'), $default_size ); ?></a></div>
		<?php } ?>
					
					<div class="um-member-card <?php if (!$profile_photo) { echo 'no-photo'; } ?>">
						
						<?php if ( $show_name ) { ?>
						<div class="um-member-name"><a href="<?php echo $iProfileurl; ?>" title="<?php echo esc_attr(um_user('display_name')); ?>"><?php echo um_user('display_name', 'html'); ?>
				
						</a> 
						<a href="<?php echo $iProfileurl; ?>" title="<?php echo esc_attr(um_user('billing_city')); ?>"><?php  
					if(!empty(um_user('billing_city','html')))
					{ 
					    echo', '.um_user('billing_city','html');
					} 
					?>
						</a>
						...
						<?php if ( UM()->roles()->um_current_user_can( 'edit', um_user('ID') ) || UM()->roles()->um_user_can( 'can_edit_everyone' ) ) { ?>
							...
							<a  style='font-size: 15px;border: none !important;
    display: block;width: 100%;line-height: 1em !important;padding: 16px 20px !important;text-decoration: none !important;text-align: center;text-transform: none !important;font-weight: normal !important;overflow: hidden;
    position: relative;transition: 0.25s;box-sizing: border-box;-moz-border-radius: 4px !important;-webkit-border-radius: 4px !important;border-radius: 4px !important;-webkit-box-shadow: 0 4px 6px rgba(50,50,93,.11), 0 1px 3px rgba(0,0,0,.08);
    box-shadow: 0 4px 6px rgba(50,50,93,.11), 0 1px 3px rgba(0,0,0,.08);opacity: 1;' href="<?php echo site_url('/account') ?>" class="um-edit-profile-btn um-alt"><?php _e( 'Edit profile','ultimate-member' ) ?></a>
							</div>
						<?php }
						...
						do_action( 'um_members_after_user_name', um_user('ID'), $args );
                                                ...
                                                $id_v = $rating_info = $starRating = '';
                                                $all_meta_for_user = get_user_meta(um_user('ID'));
